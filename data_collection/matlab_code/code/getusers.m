function [ raw_fragments ] = getusers( database_collection, startfr, endfr )
    ...
    if Mongo().find(database_collection, cursor)
       ...
       while cursor.next()
          ...
          raw_fragments{pos_in_array, 4} = datestr(transition_state.datetime('start_ts'));
          raw_fragments{pos_in_array, 5} = transition_state.string('client_addr');
          if rem(pos_in_array, 10000) == 0
              fprintf('Got %d users...\n', pos_in_array)
          end
       end
    end
    fprintf('Finished getting %d users total.\n', pos_in_array)
end
